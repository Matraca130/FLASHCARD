...
async function handleCreateFlashcard(el) {
  ...
  const difficulty =
    form.querySelector('#flashcard-difficulty')?.value || 'normal';
...
async function handleExportDeck(el) {
  ...
  if (result && !result.error) {
    showNotification(
      `Deck exportado en formato ${format.toUpperCase()}`,
      'success',
      3000
    );
  }
...
function getConfirmationMessage(action) {
  const messages = {
    'delete-deck':
      '¿Estás seguro de que quieres eliminar este deck? Esta acción no se puede deshacer.',
  ...
  return (
    messages[action] || '¿Estás seguro de que quieres realizar esta acción?'
  );
}
