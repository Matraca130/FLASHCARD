...
def calculate_fsrs(rating: int, stability: float, difficulty: float, 
                  elapsed_days: int = 0, retrievability: float = None) -> Tuple[float, float, int]:
    """
    ...
    if rating == 1:  # Again
        new_stability = w[11] * math.pow(difficulty, -w[12]) * (math.pow(stability + 1, w[13]) - 1) * math.exp((1 - retrievability) * w[14])
    else:
        # Factor de bonificación para Easy
        bonus_factor = 1.3 if rating == 4 else 1.0
        success_stability = stability * bonus_factor * (1 + math.exp(w[8]) * (11 - new_difficulty) * math.pow(stability, -w[9]) * (math.exp((1 - retrievability) * w[10]) - 1))
