...
class StudyService(BaseService):
    ...
    def start_study_session(self, user_id, deck_id, algorithm="fsrs"):
        ...
        try:
            ...
            if not available_cards:
                return self._error_response("No hay cartas disponibles para estudiar", code=404)
            ...
            if not self._commit_or_rollback():
                return self._error_response("Error al crear sesión de estudio", code=500)
    ...
    def get_next_card(self, session_id, user_id):
        ...
        try:
            ...
            session = (
                ...
                .filter_by(id=session_id, user_id=user_id, completed_at=None)  # Sesión activa
                .first()
            )
            ...
            if not available_cards:
                return self._error_response("No hay más cartas para estudiar", code=404)
    ...
    def complete_study_session(self, session_id, user_id):
        ...
        try:
            ...
            if not session:
                return self._error_response("Sesión de estudio no encontrada o ya completada", code=404)
            ...
            if not self._commit_or_rollback():
                return self._error_response("Error al completar sesión", code=500)

            # Calcular estadísticas finales
            accuracy = (session.cards_correct / session.cards_studied * 100) if session.cards_studied > 0 else 0
            ...
            return self._success_response(stats, "Sesión completada exitosamente")

        except Exception as e:
            return self._handle_exception(e, "finalización de sesión de estudio")
    ...
    def get_due_cards(self, user_id, deck_id=None, limit=50):
        ...
        try:
            ...
            query = query.order_by(Flashcard.next_review.asc().nullslast(), Flashcard.created_at.asc()).limit(limit)
            ...
            return self._success_response({"total_due": len(cards), "decks": list(cards_by_deck.values())})
    ...
    def _select_next_card(self, available_cards):
        ...
        due_cards = [c for c in available_cards if c.next_review and c.next_review <= datetime.utcnow()]
    ...
    def _apply_spaced_repetition(self, card, quality, algorithm):
        ...
        try:
            if algorithm == "fsrs":
                ...
                return self._error_response(f"Algoritmo no soportado: {algorithm}", code=400)
    ...
    def _apply_sm2(self, card, quality):
        ...
        try:
            ...
            new_ease_factor, new_interval, new_repetitions = calculate_sm2(ease_factor, repetitions, quality)

            return self._success_response((new_interval, new_ease_factor, new_repetitions))
    ...
    def _apply_anki(self, card, quality):
        ...
        try:
            ...
            if repetitions == 0:
                ...
                new_ease_factor, new_interval, new_repetitions = calculate_sm2(ease_factor, repetitions - 2, quality)
                return self._success_response((new_interval, new_ease_factor, new_repetitions))
            else:
            ...
            return self._success_response((new_interval, ease_factor, new_repetitions))
