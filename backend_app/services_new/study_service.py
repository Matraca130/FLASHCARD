...
try:
            ...
            session = self.db.session.query(StudySession).filter_by(
                id=session_id,
                user_id=user_id,
                completed_at=None  # Sesión activa
            ).first()
...
try:
            ...
            card = self.db.session.query(Flashcard).filter_by(
                id=card_id,
...
try:
            ...
            query = self.db.session.query(Flashcard).join(Deck).filter(
                and_(
                    ...
                    or_(
                        ...
                        Flashcard.last_reviewed == None  # Cartas nuevas
                    )
                )
            )
            ...
            return self._success_response({
                'total_due': len(cards),
                'decks': list(cards_by_deck.values())
            })
...
try:
            ...
            if len(due_cards) < limit:
                ...
                ).order_by(Flashcard.created_at.asc()).limit(remaining).all()
...
try:
            ...
            return self._success_response((new_interval, None, None), {
...
try:
            ...
            learning_steps = [1, 10]  # 1 minuto, 10 minutos (en días: 1/1440, 10/1440)
