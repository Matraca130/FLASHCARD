...
class BaseModel(db.Model):
    ...
    def active(cls):
        ...
        return cls.query.filter(not cls.is_deleted)
...
class Deck(BaseModel):
    ...
    def update_stats(self):
        ...
        reviews = (
            db.session.query(db.func.avg(CardReview.rating))
            .join(Flashcard, CardReview.flashcard_id == Flashcard.id)
            .filter(Flashcard.deck_id == self.id)
            .scalar()
        )
        ...
        self.total_reviews = (
            db.session.query(db.func.count(CardReview.id))
            .join(Flashcard, CardReview.flashcard_id == Flashcard.id)
            .filter(Flashcard.deck_id == self.id)
            .scalar()
            or 0
...
class Flashcard(BaseModel):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "last_review_rating >= 1 AND last_review_rating <= 5",
...
class QueryOptimizer:
    ...
    def get_user_stats(user_id, days=30):
        ...
        return {
            ...
            "accuracy_rate": round(
                ((stats.total_correct / stats.total_cards * 100) if stats.total_cards else 0),
                2,
            ),
        }
