    ...
    try:
        ...
        if chart_type == 'weekly':
            ...
            while current_date <= end_date:
                ...
                cards_studied = db.session.query(func.sum(StudySession.cards_studied)).join(Deck).filter(
            ...
            accuracy_data = db.session.query(
                Deck.id,
                Deck.name,
...
def get_progress_stats():
    ...
    try:
        ...
        cards_today = db.session.query(func.sum(StudySession.cards_studied)).join(Deck).filter(
            Deck.user_id == user_id,
            StudySession.started_at >= today_start
        ).scalar() or 0
        
        # Tiempo de estudio hoy (en minutos)
        time_today = db.session.query(func.sum(StudySession.total_time)).join(Deck).filter(
            Deck.user_id == user_id,
            StudySession.started_at >= today_start
        ).scalar() or 0
        ...
        cards_due = db.session.query(func.count(Flashcard.id)).join(Deck).filter(
            Deck.user_id == user_id,
        ...
        progress_data = {
            ...
            'accuracy': {
                ...
                'accuracy_rate': round((user.total_cards_correct / user.total_cards_studied * 100), 1) if user.total_cards_studied > 0 else 0
            }
        }
...
def get_leaderboard():
    ...
    try:
        ...
        ).group_by(User.id, User.username, User.first_name, User.last_name).order_by(
