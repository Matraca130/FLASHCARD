...
def register_error_handlers(app):
    ...
    def bad_request(error):
        ...
        log_error(f"Bad Request: {str(error)}", extra={
    ...
    def unauthorized(error):
        ...
        log_error(f"Unauthorized access: {str(error)}", extra={
            'endpoint': request.endpoint,
            'method': request.method,
    ...
    def forbidden(error):
        ...
        log_error(f"Forbidden access: {str(error)}", extra={
            'endpoint': request.endpoint,
            'method': request.method,
    ...
    def not_found(error):
        ...
        log_error(f"Resource not found: {str(error)}", extra={
            'endpoint': request.endpoint,
            'method': request.method,
            'url': request.url
        })
    ...
    def conflict(error):
        ...
        log_error(f"Conflict: {str(error)}", extra={
            'endpoint': request.endpoint,
            'method': request.method
        })
    ...
    def rate_limit_exceeded(error):
        ...
        log_error(f"Rate limit exceeded: {str(error)}", extra={
            'endpoint': request.endpoint,
            'method': request.method,
            'ip_address': request.remote_addr
        })
    ...
    def internal_server_error(error):
        ...
        log_error(f"Internal server error: {str(error)}", extra={
            'endpoint': request.endpoint,
            'method': request.method,
            'url': request.url
        })
    ...
    def handle_database_error(error):
        ...
        log_error(f"Database error: {str(error)}", exception=error, extra={
            'endpoint': request.endpoint,
            'method': request.method,
    ...
    def handle_integrity_error(error):
        ...
        log_error(f"Integrity error: {str(error)}", exception=error, extra={
            'endpoint': request.endpoint,
            'method': request.method
        })
    ...
    def handle_validation_error(error):
        ...
        log_error(f"Validation error: {str(error)}", extra={
            'endpoint': request.endpoint,
            'method': request.method,
    ...
    def handle_jwt_error(error):
        ...
        log_error(f"JWT error: {str(error)}", extra={
            'endpoint': request.endpoint,
            'method': request.method,
    ...
    def handle_generic_exception(error):
        ...
        log_error(f"Unhandled exception: {str(error)}", exception=error, extra={
            'endpoint': request.endpoint,
            'method': request.method,
    ...
    def handle_http_exception(error):
        ...
        log_error(f"HTTP exception: {error.code} - {error.description}", extra={
            'endpoint': request.endpoint,
            'method': request.method,
...
def log_and_return_error(message: str, status_code: int = 400, exception: Exception = None, extra: dict = None):
