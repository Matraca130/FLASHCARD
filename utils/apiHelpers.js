 ...
 */
export async function apiWithFallback(endpoint, fallbackData, options = {}, showFallbackMessage = true) {
  try {
    ...
    if (showFallbackMessage) {
      console.log(`API no disponible para ${endpoint}, usando datos de ejemplo`);
    }
 ...
 */
export async function performCrudOperation(operation, successMessage, errorMessage) {
  try {
 ...
 */
export async function loadDataWithRetry(endpoint, fallbackData, maxRetries = 3, retryDelay = 1000) {
  let lastError;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        console.log(`Intento ${attempt} fallido, reintentando en ${retryDelay}ms...`);
        await new Promise(resolve => setTimeout(resolve, retryDelay));
      }
    }
  }
  
  console.log(`Todos los intentos fallaron, usando datos de fallback. Error: ${lastError.message}`);
  return fallbackData;
}
...
export const FALLBACK_DATA = {
  ...
  decks: [
    ...
    { id: 3, name: 'Historia Mundial', card_count: 75, last_studied: '2024-01-13' }
  ],
