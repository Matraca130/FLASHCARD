...
async function initializeAllCharts() {
  const chartInitializers = [
    { id: 'weeklyProgressChart', init: initializeWeeklyProgressChart },
    {
      ...
      init: initializeCategoryDistributionChart,
    },
        ...
        datasets: [
          {
            ...
            backgroundColor:
              categoryData.colors || CHARTS_CONFIG.colorPalettes.blue,
            borderWidth: 2,
            borderColor: '#ffffff',
            hoverBorderWidth: 3,
            hoverOffset: 10,
        ...
        datasets: [
          {
            ...
            data: [
              ...
              100 -
                (performanceData.speed[performanceData.speed.length - 1] / 60) *
                  100, // Invertir velocidad
...
function calculateConsistency(accuracyData) {
  ...
  const variance =
    accuracyData.reduce((acc, val, i, arr) => {
      if (i === 0) {
        return 0;
      }
...
export async function updateProgressChart(data = null, period = '7d') {
  try {
    ...
    const progressChart =
      chartInstances.get('studyProgress') ||
    ...
    if (progressChart) {
      ...
      if (data.target && progressChart.data.datasets[1]) {
        progressChart.data.datasets[1].data = Array(data.labels.length).fill(
          data.target
        );
      }
...
export async function updateAccuracyChart(data = null, period = '7d') {
  try {
    ...
    const accuracyChart =
      chartInstances.get('accuracyTrend') ||
    ...
    if (accuracyChart) {
      ...
      if (data.target && accuracyChart.data.datasets[1]) {
        accuracyChart.data.datasets[1].data = Array(data.labels.length).fill(
          data.target
        );
      }
...
export async function updateChartPeriod(period = '7d') {
  try {
    ...
    console.log(
      `✅ Período actualizado: ${successful}/${total} gráficos actualizados exitosamente`
    );
